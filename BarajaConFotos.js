...
function generarCarta(){
    let cartaID = Math.trunc(Math.random()*41)
    console.log(cartaID)
    let cartaRuta = cartas[cartaID]["ruta"]
    let numero
    if (cartas[cartaID]["num"]==1){
        numero = "As de"
    }
    else if(cartas[cartaID]["num"]==8){
        numero = "Sota de"
    }else if(cartas[cartaID]["num"]==9){
        numero = "Caballo de"
    }else if(cartas[cartaID]["num"]==10){
        numero="Rey de"
    }else{
        numero = cartas[cartaID]["num"]+" de"
    }
    let palo = cartas[cartaID]["palo"]+"s"
    document.querySelector("h1").innerHTML=numero+" "+palo
